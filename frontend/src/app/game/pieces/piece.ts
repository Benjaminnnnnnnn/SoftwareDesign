...
export default abstract class Piece {
  ...
  public giveItem(_item : Item): void {
    if (!this.item){ // if there is no item
  ...
  public async getSprite(): Promise<PIXI.Sprite | null>
  {
    ...
    if(!images[this.id]){ console.log("no image for given id")// if no image is associated with id, load default
      try { console.log("load default");
        ...
        if (!this.item){console.log("no item")}
      else{
    ...
    else{ // otherwise, load corresponding image
    try {
